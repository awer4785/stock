...
$sqls = "UPDATE product SET active='3' Where quantity <= 4 AND quantity != 0";
$results = $connect->query($sqls);

$sqlss = "UPDATE product SET active='2' Where quantity <= 0 ";
$resultss = $connect->query($sqlss);

$sqlsss = "UPDATE product SET active='1' Where quantity > 4";
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[7] == 1 ){
 		// activate member
 		$active = "<label class='label label-success'>สามารถเบิกได้</label>";
 	} elseif($row[7] == 2) {
 		// deactivate member
 		$active = "<label class='label label-danger'>ไม่สามารถเบิกได้</label>";
 	} // /else
	 else{
		$active = "<label class='label label-warning'>กรุณาเติมพัสดุ</label>";
