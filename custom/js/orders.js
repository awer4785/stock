...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
	 			var quantityId = quantity[x].id;
		    if(quantity[x].value == '' && quantity[x].value < quantity){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> Product Name Field is required!! </p>');
		    	$("#"+quantityId+"").closest('.form-group').addClass('has-error');	    		    		    	
	      } else {      	
		    	$("#"+quantityId+"").closest('.form-group').addClass('has-success');	    		    		    		    	
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && description ) {
	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'</td>'+
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="left[]" id="left'+count+'" autocomplete="off" disabled="true" class="form-control" />'+
					'<input type="hidden" name="leftValue[]" id="leftValue'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#left"+row).val(response.quantity);
					$("#leftValue"+row).val(response.quantity);
